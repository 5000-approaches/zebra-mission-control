...
export default function Home() {
  ...
  async function handleSend(e?: FormEvent) {
    ...
    try {
      ...
      // Component unmounted mid-stream — stop silently.
    } finally {
  ...
  return (
    ...
    >
      <div
        className="px-6 py-5 border-b flex-shrink-0 flex flex-col gap-4"
        style={{ borderColor: "var(--page-border)" }}
      >
        <AgentHeader firstName={firstName} />
        <McpServersPanel />
      </div>

      <div ref={scrollRef} onScroll={trackScroll} className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages.length === 0 && (
          <p className="text-sm text-center mt-12" style={{ color: "var(--page-text)", opacity: 0.4 }}>
            Ask a question like &ldquo;What is our billable forecast for April?&rdquo; or &ldquo;Which projects earned most last month?&rdquo;
          </p>
        )}
        {messages.map((msg, i) => (
          <div
      ...
      >
        ...
        <div className="flex gap-3 items-end">
          ...
          <textarea
            ...
            onPaste={handlePaste}
            placeholder="Ask the Zebra Agent…"
            aria-label="Ask the Zebra Agent"
            disabled={loading}
            rows={1}
